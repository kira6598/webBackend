...
import com.example.webbanhangfinal.auththenticate.Register.Email.ValidatorEmail;
...
import com.example.webbanhangfinal.auththenticate.UserManagerment.Role;
...
public class RegisterService{
    ...
    public String registerRequest(@RequestBody RegisterRequest request) throws AppUserException{
        if(!validatorEmail.test(request.email())) {
            ...
            String token = appUserService.SignUpUser(
                new AppUser(request.firstName(), request.lastName(), request.email(), Role.USER, request.password())
            );
         
            String link = "http://localhost:8080/auth/confirm?token="+token;
            emailService.Send(request.email(), buildEmail(request.lastName(), link));
            return "Success";
        }
    ...
    public String confirmToken(@RequestParam String token) {
        ...
        if(confirmationToken.getConfirmedAt()!=null){
            new IllegalStateException("your account already active");
        }
        LocalDateTime expired= confirmationToken.getExpridateAt();
        if(expired.isBefore(LocalDateTime.now())){
            new IllegalStateException("your token is expired");
        }
        // enable user and Set time you confirm if token is clicked;
